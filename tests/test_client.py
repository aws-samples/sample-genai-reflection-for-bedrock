...
def example_boto_config():
    return Config(region_name="us-east-1", signature_version="v4", retries={"max_attempts": 10, "mode": "standard"})
...
def should_instantiate_using_client_config(mock_boto_client, mock_runtime_client, example_boto_config):
    mock_boto_client.return_value = mock_runtime_client
    bedrock_hive = client.BedrockHive(client_config=example_boto_config)
    mock_boto_client.assert_called_once_with(service_name=client._RUNTIME_CLIENT_NAME, config=example_boto_config)
